...
class EmbedYouTube extends HTMLElement {
	...
	autoLoadIframe() {
		...
		this.observerIframe = new IntersectionObserver((entryList, observer) => {
		...
		this.observerIframe.observe(this);
	}
	...
	autoPauseVideo() {
		...
		this.observerVideo = new IntersectionObserver((entryList) => {
	...
	/**
	 * The `disconnectedCallback` function disconnects observers for an iframe and a video element if they exist.
	 */
...
if (!customElements.get("embed-youtube")) {
